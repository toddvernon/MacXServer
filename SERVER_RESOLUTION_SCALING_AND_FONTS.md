...
This table is the **seed** for a user-editable file at `~/.macxserver-fonts` (as of 2026-05-23). `FontResolver.installMappings` writes the seed on first run and loads the user's edits thereafter. The Mac chrome's "Edit Font Mappings…" menu opens a SwiftUI editor for it. Revert to Defaults overwrites the file with the seed. The bundled seed lives in `Sources/SwiftXServerCore/DefaultFontMappings.swift`; this table is the canonical spec for what that seed contains.
