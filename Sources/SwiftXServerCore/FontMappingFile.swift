...
//
// The `->` separates the family name from the Mac font; the trailing
// `mono`/`prop` token separates the Mac font from the spacing kind. This
// supports multi-word X family names ("new century schoolbook") and
// multi-word Mac font names ("Helvetica Neue") without ambiguity.
//
...
public struct FontMappingFile: Sendable {
    ...
    /// plus its monospace flag, or nil if the name isn't in the table.
    public func resolve(family: String) -> (macFont: String, isMonospace: Bool)? {
        let normalized = family.lowercased()
        for m in mappings where m.family == normalized {
            return (m.macFont, m.isMonospace)
        }
        return nil
    }
    ...
    /// fallback; anything else → proportional fallback.
    public func fallback(spacing: String) -> (macFont: String, isMonospace: Bool) {
        switch spacing.lowercased() {
        case "c", "m": return (fallbackMono, true)
        default:       return (fallbackProp, false)
        }
    }
        ...
        for rawLine in text.split(separator: "\n", omittingEmptySubsequences: false) {
            ...
            default:
                mappings.append(FontMapping(
                    ...
                    isMonospace: parsed.isMonospace
                ))
            }
        }
    ...
    /// isMonospace). Format: `<family>  ->  <mac-font>  mono|prop`.
    /// The `->` separates family from mac font (so multi-word families
    /// like "new century schoolbook" work); the trailing `mono`/`prop`
    /// token separates mac font from spacing kind.
    private static func parseLine(_ line: String) -> (family: String, macFont: String, isMonospace: Bool)? {
        // Tokenize on runs of whitespace.
        let tokens = line.split(whereSeparator: { $0 == " " || $0 == "\t" }).map(String.init)
        guard tokens.count >= 4 else { return nil }   // need at least family, ->, mac, kind

        // Locate the arrow separator.
        guard let arrowIdx = tokens.firstIndex(of: "->") else { return nil }
        guard arrowIdx >= 1, arrowIdx <= tokens.count - 3 else { return nil }
        // arrowIdx >= 1 → at least one family token before the arrow
        // arrowIdx <= count-3 → at least one mac-font token + the kind after

        // Last token is the spacing kind.
        let last = tokens.last!.lowercased()
        let isMono: Bool
        switch last {
        case "mono": isMono = true
        case "prop": isMono = false
        default: return nil
        }
        ...
        // Mac font: tokens[(arrowIdx + 1)..<(count - 1)], joined with spaces.
        let macFont = tokens[(arrowIdx + 1)..<(tokens.count - 1)]
            .joined(separator: " ")
        guard !macFont.isEmpty else { return nil }

        return (family, macFont, isMono)
    }
}
