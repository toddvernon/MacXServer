...
- **Public opcode reference:** `OPCODES_PUBLIC.yaml` at the repo root is the source-of-truth data file that drives the public opcode-reference page on macxserver.com (rendered by Hugo from `macxserver-hugo/data/opcodes.yaml`, which is a symlink to it). The `description` fields are immortal and basically never change. The `coverage` fields ARE mutable and must stay in lockstep with `OPCODE_STATUS.md`. When you update an opcode's status row, update the matching YAML `coverage` line in the same commit. `scripts/check-opcode-coverage-drift.sh` flags any mismatch; run it during milestone audits.
